...
PauseMenuState::PauseMenuState(GameEngine& game, sf::Font& font)
	: game(game), font(font), title(font)
{
	if (!buttonTexture.loadFromFile("Textures/MenuSprites/OnlyPause.png"))
	{
		std::cerr << "ERROR Class PauseMenuState: Failed to load button texture" << std::endl;
	}
	...
	for (int i = 0; i < options.size(); ++i)
	{
		...
		options[i].addAnimation("HOVER", 1, sf::IntRect({ 0, Constants::PAUSE_SPRITE_SIZE_Y * (3 * i + 1) }, Constants::PAUSE_SPRITE_SIZE), 0.5f, false);
...
void PauseMenuState::handleInput(GameEngine& engine, std::optional<sf::Event>& event)
{
...
void PauseMenuState::update(GameEngine& engine, sf::Time deltaTime)
{
...
void PauseMenuState::resizeWindow(sf::RenderTarget& window)
{
